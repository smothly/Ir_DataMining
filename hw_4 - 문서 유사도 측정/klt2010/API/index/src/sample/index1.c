/*--------------------------------------------------------------------------
	색인어 추출 함수 get_stems()를 호출하는 예제 프로그램
----------------------------------------------------------------------------
HAM 라이브러리 안에 있는 색인어 추출 함수 get_stems() 사용방법
아래 순서에 의해 라이브러러 사용법을 test해 보기 바랍니다.

  1. index1.c와 libham.a를 subdirectory 'hdic'이 있는 곳으로 옮긴다.

  2. index1.c를 compile한다.
     $ cc -c index1.c

  3. HAM 라이브러리 libham.a와 함께 link한다.
     $ cc -o index1 index1.o -L. libham.a

  4. 실행파일 'index1'를 실행하여 확인한다.

혹시, 사전을 찾지 못하는 error가 나면, 함수 open_HAM_index()에서 사전파일 경로인
"./hdic/"이 잘못된 경우로 "hdic/ham2000.ini"의 맨 앞 부분의 사전경로에 사전파일 확인.

함수 get_stems()는 HAM 사전들을 사용하기 때문에
...
main()
{
	...
	puts("한글 문장을 입력하시오.");
	while (gets(sent)) {
		nkeys = get_stems(sent, keys, &mode);	/* multi-threading --> get_stems_TS() 사용 */
